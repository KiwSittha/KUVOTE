...
// ================= LOAD DATA =================
  ...
  const fetchData = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        console.error("ข้อมูลที่ได้มาไม่ใช่ Array:", data);
        setCandidates([]); 
      }
    } catch (error) {
      console.error("Fetch error:", error);
      setCandidates([]); // กันเหนียวกรณี API ล่ม
    }
  };
  ...
  const filteredCandidates =
    filter === "all"
      ? candidates
      : candidates?.filter((c) => c.status === filter) || []; // ✅ เติม ?. และ || []

  const countStatus = (status) =>
    candidates?.filter((c) => c.status === status)?.length || 0; // ✅ เติม ?. และ || 0
